...
const Form = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        const result = await userRegistration({ name, email, phone, password, confirmPassword });
    ...
    <form onSubmit={formik.handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        {formik.touched.confirmPassword && formik.errors.confirmPassword ? (
          <div className="text-red-900 text-sm">{formik.errors.confirmPassword}</div>
        ) : null}
      </div>

      <LoadingButton type="submit" loading={loading} className="bg-slate-200 text-indigo-800 hover:bg-slate-200 w-full py-2">
        Submit
      </LoadingButton>

      {error && <div className="text-red-900 text-sm font-semibold mt-2">{error}</div>}
    </form>
  );
};
